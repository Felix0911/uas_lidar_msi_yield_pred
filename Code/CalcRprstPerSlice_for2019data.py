...
for pts_tstp in pts_time_stamps:
    ...
    results = []
    for i in range(seg_labels.max()):
        seg_pts = las_pts[seg_labels==i+1]
        if len(seg_pts) == 0:
            print(f'Row {i} is empty. Zeros were saved!!') #One of the rows was baren due to soil quality.
        ...
        poisson_area, poisson_vol, vol_ratio = cp_pts.calcPoissonRecVolume(pts_dir[:-4] + '_temp.stl', 
                                                                           ifVisual=False)
        LAI = cp_pts.calcLidarLAI(h_th=0.25)
        print(f'Row {i}.')
        results.append([i+1, h_90pct, sumZ, num, cumInt, topArea,
